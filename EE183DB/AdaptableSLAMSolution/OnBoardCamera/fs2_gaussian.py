...
class Particle:
    ...
    def __init__(self, N_LM):
        ...
        #self.lmP = R
        self.lmP = np.tile(self.lmP, (N_LM,1))
...
def predict_particles(particles, u):
    ...
    return particles
def add_new_lm_mod(particle, z, R, i, debug_coords, prob):
...
def update_landmark(particle, z, Q, i):
    ...
    if (i == 0 and PRINT_DEBUG):
        ...
        print("zp", zp)
        print("z", z)
        print("particle 0, lm_id: %d new (x,y):" %(lm_id))
        ...
        for k, loc in enumerate(np.hsplit(locations, locations.shape[1])):
            ...
            for ip in range(N_PARTICLE):
                ...
                if max_ip == -1:
                    ...
                    particles[ip], lm_id = add_new_lm_mod(particles[ip], loc, R, ip, debug_coords[k], max_ip)
                    if lm_id == -1:
...
def update_with_observation(particles, z):
    ...
    for iz in range(len(z[0, :])):
        ...
        for ip in range(N_PARTICLE):
            ...
            particles[ip].w *= w
            particles[ip] = update_landmark(particles[ip], z[:, iz], R, ip)
            particles[ip] = proposal_sampling(particles[ip], z[:, iz], R, ip)
    return particles
        ...
        while(SIM_LENGTH >= sim_time):
            ...
            if show_animation:
                ...
                for i in range(N_PARTICLE):
                    ...
                    print(particles[i].lm[:particles[i].seen])
